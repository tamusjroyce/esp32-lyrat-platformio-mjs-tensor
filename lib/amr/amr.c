...
#ifdef CONFIG_CHOICE_AMR_WB
    struct amr_config config = {second_recorded, evt, pipeline, amr_encoder, i2s_stream_reader, fatfs_stream_writer};
#elif defined CONFIG_CHOICE_AMR_NB
    struct amr_config config = {second_recorded, evt, pipeline, amr_encoder, i2s_stream_reader, fatfs_stream_writer};
#else
    struct amr_config config = {second_recorded, evt, pipeline, NULL, i2s_stream_reader, fatfs_stream_writer};
#endif
...
bool amr_process(struct amr_config *config)
{
	...
	if (audio_event_iface_listen(config->evt, &msg, 1000 / portTICK_RATE_MS)
			!= ESP_OK) {
		config->second_recorded++;
		ESP_LOGI(TAG, "[ * ] Recording ... %d", config->second_recorded);

		if (config->second_recorded >= RECORD_TIME_SECONDS) {
	...
	if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT
			&& msg.source == (void*) config->i2s_stream_reader
...
void amr_cleaning_up(struct amr_config *config)
{
	...
	audio_pipeline_terminate(config->pipeline);
	/* Terminate the pipeline before removing the listener */
	audio_pipeline_remove_listener(config->pipeline);
	/* Stop all peripherals before removing the listener */
	esp_periph_stop_all();
	audio_event_iface_remove_listener(esp_periph_get_event_iface(), config->evt);
	/* Make sure audio_pipeline_remove_listener & audio_event_iface_remove_listener are called before destroying event_iface */
	audio_event_iface_destroy(config->evt);
	/* Release all resources */
	audio_pipeline_unregister(config->pipeline, config->amr_encoder);
	audio_pipeline_unregister(config->pipeline, config->i2s_stream_reader);
	audio_pipeline_unregister(config->pipeline, config->fatfs_stream_writer);
	audio_pipeline_deinit(config->pipeline);
	audio_element_deinit(config->fatfs_stream_writer);
	audio_element_deinit(config->i2s_stream_reader);
	audio_element_deinit(config->amr_encoder);
	esp_periph_destroy();
}
